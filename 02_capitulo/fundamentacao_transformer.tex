...
e o valor são todos derivados da \textit{mesma} sequência de entrada. Em outras palavras,
ao processar uma frase, cada palavra pergunta a si mesma: ``quais outras palavras desta
frase são relevantes para minha interpretação?''. Considere a frase ``O gato que estava
...
quando $d_k$ é grande --- magnitudes muito altas causam gradientes muito pequenos após a
...
degradação significativa da qualidade das predições \cite{Michel2019}. Essa propriedade
...
Os dois componentes internos de cada bloco merecem descrição detalhada. O primeiro é a
...
transformações lineares com uma função de ativação não linear entre elas: a função GELU
(\textit{Gaussian Error Linear Unit}) nos modelos GPT, ou ReLU em outras variantes. A
...
raciocínio básico e muito mais \cite{Goodfellow2016}.
...
predições \cite{Patterson2021}. Esse potencial de redução do consumo energético via poda
